...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    public void insert(T item, double priority) {
        ...
        /* TODO: Your code here! */
        ArrayHeap<T> pq = new ArrayHeap<>();
        ArrayHeap<T>.Node innerNode = pq.new Node(item, priority);
        contents[++size] = innerNode;
        swim(size);
    }
    ...
    public void testSwim() {
        ...
        pq.size = 7;
        for (int i = 1; i <= 7; i += 1) {        	
        	ArrayHeap<String>.Node innerNode = pq.new Node("x" + i, i);
        	pq.contents[i] = innerNode;
//            pq.contents[i] = pq.getNode("x" + i, i);   //Node("x" + i, i);
        }
